...
| TLS | `WiFiClientSecure` insecure-mode handshake + **post-handshake pin check**: `peerMatchesPinned()` byte-compares the peer cert against the NVS PEM (`_bridgeCertPem`) before anything — including the app key — is sent. (`setCACert()` CA-mode is impossible here: hostname verification is always enforced, we dial by IP, and the bridge cert's CN is the bridge ID — found live July 15.) |
...
The connection is opened by `sseConnect()` at the end of `setup()`, after WiFi, NTP, VEML7700, cert refresh, and `bootstrapLightStates()`. From then on it's owned and drained continuously by the Core-0 `sseTask` (N1 Stage 1) — reconnects included, each followed by a `bootstrapLightStates(true)` resync that enqueues synthetic edge events for on/off flips missed while the stream was down (N1 Stage 3).
...
`bootstrapLightStates()` uses `setInsecure()` deliberately (July 15): HTTPClient transmits on connect, so post-handshake pin verification can't precede the request — bridge identity is instead verified at boot by `ensureBridgeCert()`'s probe and continuously by the SSE stream's pin check.
...
**Read path:** `cachedLight(idx)` returns a `LightState` copy. Used by `triggerWake()`, `dispatchSseLight()`'s re-arm all-off check (N1 Stage 3 — the old `checkFloorState()` poll and `lastFloorOn` accumulator are deleted), the state-transition resync points (`overheadsOn = cachedLight(LIGHT_CEIL_1).on`, `chestOn = cachedLight(LIGHT_CHEST).on`), the setup flourish (`saved = cachedLight(LIGHT_CHEST)`), and `noteRecentPut()` for the trajectory `priorBri`/`priorCt`.
...
Current approach: every incoming event is classified into one of ten `EchoOutcome` buckets (the tenth, `SyntheticEdge`, is generated by reconnect resync rather than the bridge — N1 Stage 3). The override path is reached only when the event is off-trajectory of every live `recentPuts` slot, off-cache by more than `TRAJECTORY_TOLERANCE_*`, *and* not a stale revert of one of our own recent PUTs. Anything else is an echo (or a skip) and does not trigger SOFT_PAUSE.
...
const int           ECHO_OUTCOME_COUNT       = 10;        // size of echoOutcomeCounts[] histogram
```
