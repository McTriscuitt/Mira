...
- **N1 Stage 5 — dashboard networking task (July 16, flashed + boot-verified 21:11)** — `netTask` (Core 0, prio 1) owns all Railway HTTP: `sendLog()` now truncate-copies into a drop-oldest `logQueue` (12× `char[120]`, non-blocking); the status POST split into main-task `queueDashboardStatus()` (POD `StatusSnapshot` → depth-1 queue, `xQueueOverwrite`/latest-wins) + netTask `postDashboardStatus()`; the command poll moved off the tick to netTask every `NET_CMD_POLL_MS` (5 s — command latency was ~30 s), enqueuing `DashboardCmd` events that `dispatchDashboardCmd()` applies on the main task (ack-after-enqueue, at-least-once). The tick's only network I/O is now bridge PUTs. Heap/stack soak telemetry (`heap_free`/`sse_stack_free`/`net_stack_free`) rides every `/api/status` POST — `app.py` gained matching columns + migration and owner-only `/api/telemetry/history` (48 h default), so long soaks no longer need the laptop/serial tether. Each applied command also pushes a fresh snapshot (~100 ms), shrinking the June 11 ack-before-status window to near zero. Boot capture: net stack floor 6,624/12,288, heap ~8.5 MB baseline, netTask POSTs 200, first live DashboardCmd (a queued wind-down) applied ~5 s after boot. Prerequisite for N2's long-poll. Stage 4 was live-verified the same evening (all four checklist items passed).
- **N1 Stage 4 — ramps & soft-pause expiry on soft timers (July 16, flashed)** — `TIMER_WAKE_STEP` / `TIMER_WINDDOWN_STEP` / `TIMER_RESUME_STEP` (30 s periodic) + `TIMER_PAUSE_EXPIRY` (one-shot) decouple ramp advancement and pause expiry from the LuxTick; the tick's only state work is now the NORMAL curve. No per-site arm/disarm: `syncSoftTimers()` reconciles the active-timer set with `(state, pauseResumeActive)` every `loop()` pass (see `N1_MIGRATION.md` Decisions Log, July 16). Ramp timers arm due-now → dashboard WIND_DOWN/WAKE commands and the SSE wake edge start their ramp in ~50 ms instead of at the next 30 s boundary. `tickSoftPause()` → `expireSoftPause()` (stale-fire guard vs. racing extend commands); resume ramp body moved from `tickNormal()` to `advanceResumeRamp()`. `ECHO_TRACE` off again (Stage 3 soak done, accepted July 16). Stage 3 marked soak-verified the same day.
- **Evening schedule shift (July 15, shipped `56d2841` + `6b2646d`)** — wind-down may now complete as early as **8:30 PM**: new `WIND_DOWN_GATE_HOUR 19` (stable-lux counter gate, was hard-coded 21) and `LOCKOUT_RESET_MIN_OF_DAY 1230` (re-arm gate, minute-granular, was `LOCKOUT_RESET_HOUR 21`). Lux still rules wind-down; the hours are floors. **Deferred:** re-tune the `lux <= 10` wind-down gate for the sensor's desk (non-windowsill) placement — see User Notes.
- **N1 Stage 3 + quick-fix batch (July 15, shipped `69aa744` + `6b478b6`)** — (1) **G22/G23**: wake trigger + lockout re-arm moved from the per-tick `checkFloorState()` poll to SSE edge dispatch (`dispatchSseLight()`); `lastFloorOn` and the poll deleted; on/off edge bits ride in every queued SseLight event's `flags`; SSE reconnects resync the cache and enqueue `SyntheticEdge` events for missed flips. (2) **R3 revised**: `setCACert` pinning is impossible (CN mismatch dialing by IP — found live; the old NVS probe had silently re-fetched every boot); replaced with post-handshake byte-compare pinning (`peerMatchesPinned()`) on the boot probe + SSE stream. (3) **R10**: soft-pause resume seeds from the floor lamp's cached state. (4) **R11**: firmware drains up to 8 queued dashboard commands per tick. `ECHO_TRACE` temporarily on for the Stage 3 soak.
- **Soft-pause slider direction flip + pending pin (July 14, shipped `52f17b6`)** — slider value in `index.html` is now minutes *elapsed* (`max − remaining`) so the thumb runs left→right like the wind-down/wake sliders; drag/extend/preview handlers convert back to the remaining minutes `SET_SOFT_PAUSE_REMAINING` expects (wire protocol unchanged). Added `pauseSeekPending` — the same pending-pin the other step-seek sliders had — fixing the mobile snap-back where a status re-render reverted a just-dragged slider to the stale snapshot.
...
- **State machine** — `enum class State` with 6 states; per-state switch inside `dispatchLuxTick()` (N1 Stage 2); dedicated tick functions per state
- **Polling loop** — reads lux every 30 s on `:00/:30` wall-clock boundaries (N5/`ALIGNED_TICKS`), sends to active bulbs when `shouldUpdate` (bri change > `STATE_TOLERANCE_BRI` or ct change > `STATE_TOLERANCE_CT`)
- **Dashboard logging** — `sendLog()` queues log lines for the Core-0 `netTask` to POST to the Railway dashboard (`/api/log`) — non-blocking, fixed `char[120]` messages, drop-oldest on overflow (N1 Stage 5; was a synchronous main-task POST); logs startup, lux/bri/ct updates, wind-down trigger/milestones/completion, wake start/milestones/completion, soft pause trigger/resume, state transitions
...
- **Wind-down** — stable lux counter (lux ≤ 10, hour ≥ `WIND_DOWN_GATE_HOUR` 19, 60 readings — earliest completion 8:30 PM, later if the room is still bright); 60-min linear dim on floor+chest+dresser to floor bri/ct=warm; **chest off at the midpoint (step 60, cache-guarded so a dashboard step-seek past 60 still kills it), dresser off at the end (step 120)**; floor lamp holds at floor brightness as the night-light anchor; `dynamics.duration=30000` matches poll interval for seamless gradient; `tickWindDown()` in `main.cpp`
...
- **Morning lockout (SSE-edge-driven since Stage 3, July 15)** — wake trigger and lockout re-arm live in `dispatchSseLight()`: a floor-lamp rising edge during LOCKED_OUT fires `triggerWake(lastLux)` within ~100 ms; any falling edge after `LOCKOUT_RESET_MIN_OF_DAY` (8:30 PM) with floor/chest/dresser/ceil1 all off (cache) re-arms from any state except HARD_OFF, resetting `stableLuxCount`/`windDownStep`, clearing `excludedLight[]`, and cancelling any pause/resume ramp. The old per-tick `checkFloorState()` poll and its `lastFloorOn` accumulator (with seven sync sites) are deleted; edge bits (prevOn/nowOn) ride in each queued event's `flags`, and SSE reconnects resync the cache + enqueue synthetic edges for flips missed while the stream was down
...
- **SSE event stream** — persistent HTTPS connection (`sseClient` global) owned by the Core-0 `sseTask`, which drains it continuously; pushes bridge state changes into `lightCache[LIGHT_COUNT]` and runs override/edge classification in `handleLightUpdate()`. `bootstrapLightStates()` seeds the cache at startup and resyncs it (with synthetic edge events) after every reconnect. Pin-checked via `peerMatchesPinned()` before the app key is sent. Full reference in `Markdowns/SSE.md`.
